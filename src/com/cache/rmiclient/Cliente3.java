...
public class Cliente3 extends UnicastRemoteObject implements RMIInterfaceCliente, Serializable{
	protected Cliente3() throws RemoteException {
		super();
		// TODO Auto-generated constructor stub
	}

	private static RMIInterface look_up;
	static List<Usuario> cacheCliente3 = new ArrayList<Usuario>();
	
	public static void main(String[] args) throws MalformedURLException, RemoteException, NotBoundException {
		

		look_up = (RMIInterface) Naming.lookup("//localhost:1100/MyServer"); //Servidor
        Naming.rebind("//localhost:1099/Cliente3", new Cliente3());
	...
	public static boolean validaCache(int id) {
		...
		for (Usuario usuario : cacheCliente3){
	...
	public static void imprimiCache() {
		...
		System.out.println("Cache Cliente 3:");
		
		if(cacheCliente3.isEmpty()) {
	...
	public void updateCacheById(Integer id,String novocpf) throws RemoteException {
		...
		for (int i = 0 ; i < cacheCliente3.size(); i++) {	
			   if(cacheCliente3.get(i).getId_usuario() == id) {
				   cacheCliente3.get(i).setCpf_usuario(novocpf);
