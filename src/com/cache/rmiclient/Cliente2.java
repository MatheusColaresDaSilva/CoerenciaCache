...
public class Cliente2 extends UnicastRemoteObject implements RMIInterfaceCliente, Serializable{
	protected Cliente2() throws RemoteException {
		super();
		// TODO Auto-generated constructor stub
	}

	private static RMIInterface look_up;
	static List<Usuario> cacheCliente2 = new ArrayList<Usuario>();
	
	public static void main(String[] args) throws MalformedURLException, RemoteException, NotBoundException {
		

		look_up = (RMIInterface) Naming.lookup("//localhost:1100/MyServer"); //Servidor
        Naming.rebind("//localhost:1102/Cliente2", new Cliente2());
	...
	public static boolean validaCache(int id) {
		...
		for (Usuario usuario : cacheCliente2){
	...
	public static void imprimiCache() {
		...
		System.out.println("Cache Cliente 2:");
		
		if(cacheCliente2.isEmpty()) {
	...
	public void updateCacheById(Integer id,String novocpf) throws RemoteException {
		...
		for (int i = 0 ; i < cacheCliente2.size(); i++) {	
			   if(cacheCliente2.get(i).getId_usuario() == id) {
				   cacheCliente2.get(i).setCpf_usuario(novocpf);
